...
Mario.WinState.prototype.CheckForChange = function(context) {
    if (this.waitTime <= 0) {
        if (this.wasKeyDown && !Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.S)) {
            // Weiterleitung zur neuen Seite mit playerName als URL-Parameter
            if (name) {  // Prüfen Sie, ob playerName gesetzt wurde
                window.location.href = "code/zertifikat.html?playerName=" + encodeURIComponent(name);
            } else {
                // Optionale Handhabung, falls playerName nicht gesetzt wurde
            }
        }
    }
};
